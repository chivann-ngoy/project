...
public class StudentDaoImpl implements StudentDao {
    ...
    @Override
    public boolean daleteById(Long id) {
    ...
    public Student updateById(Long id, Student student) {
        for (int i = 0;i< StudentDb.students.size();i++){
            if (StudentDb.students.get(i).equals(id)){
    ...
    public List<Student> searchByName(String name) {
        ...
        return StudentDb.students.stream().filter(stu -> stu.getFullName().toLowerCase()
                .contains(name.toLowerCase())).toList();
    }

    @Override
    public StudentMapper save(Student student) {
        return null;
    }
}